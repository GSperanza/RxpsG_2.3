...
XPSprocess <- function(Object, Object_name = as.character(substitute(Object))) {
...
## to update fit remove Component@Fit and make the sum of Component@ycoor including the newone
   ...
   do.before.close <- function(...) {
       setGraphicsEventHandlers(which = dev.cur(), onMouseDown = 1)  #stops the Graphics.Event.Handler
...
#----- TAB1: Baseline -----
  ...
  BaselineType1 <- gradio(items=c("linear", "polynomial", "spline"), selected=1, spacing=1, horizontal = TRUE, handler = function(h, ...){
                                   ...
                                   if (BType == "spline") {
                                      ...
                                      draw.plot()
                                      delete(T1Frame3, T1group2)
                                      T1group2 <<- ggroup(horizontal=TRUE, spacing=1, container = T1Frame3)
  ...
  BaselineType3 <- gradio(items=c("2P.Tougaard", "3P.Tougaard", "4P.Tougaard"), selected=-1, spacing=1, horizontal = TRUE, handler = function(h, ...){
                                   ...
                                   if (BType=="2P.Tougaard") {
                                      ...
                                      T1group2<<-ggroup(horizontal=FALSE, spacing=1, container = T1Frame3)
                                      glabel("   ", spacing=1, container=T1group2) #add two space rows
                                      glabel("   ", spacing=1, container=T1group2)
                                      add(T1Frame3, T1group2)
                                      reset.baseline()
                                      Marker <<- list(Points=point.coords, col=2, cex=1.6, lwd=1.5, pch=10)
...
#  addSpring(Pgroup1)
  ...
  gbutton("SAVE & EXIT", container = Pgroup1, handler = do.before.close  )
...
#--- Set interrupts on graphic window -----
    ...
    while(WhileExit > 0){
         ...
         if(length(Focus) == 0){  #focus is on the graphic window
             eventEnv <- getGraphicsEvent(prompt = "Press Left Mouse Button",  #get the interrupt when the mouse buttons are pressed
